...
compose_recirculate_action__(struct xlate_ctx *ctx, struct ofpbuf *stack,
                             ...
                             struct ofpact *ofpacts, uint8_t table)
{
...
compose_recirculate_action(struct xlate_ctx *ctx)
{
    ...
    compose_recirculate_action__(ctx, &ctx->stack, ctx->recirc_action_offset,
                                 ctx->action_set.size, ctx->action_set.data, 0);
...
compose_conntrack_action(struct xlate_ctx *ctx, struct ofpact_conntrack *ofc)
{
    ...
    /* XXX: Factor out this first part of this function into commit_ct_action()
     * in odp-util? The put_connhelper() and friends can all live statically
     * there. */
    ...
    if (ofc->recirc_table == NX_CT_RECIRC_NONE) {
        ...
        compose_recirculate_action__(ctx, NULL, 0, 0, NULL, ofc->recirc_table);
    }
}
