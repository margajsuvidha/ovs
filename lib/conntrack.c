...
static void conn_keys_lookup(struct conntrack *, struct conn_key_lookup *keys,
                             unsigned long maps, unsigned bucket, long long now);
            ...
            bool commit, long long now)
{
    ...
    uint8_t state = OVS_CS_F_TRACKED;
    unsigned bucket = key->hash % CONNTRACK_BUCKETS;
...
void conntrack_run(struct conntrack *ct) OVS_NO_THREAD_SAFETY_ANALYSIS
{
    unsigned bucket_ext = ct->purge_bucket_ext % CONNTRACK_BUCKETS;
    uint32_t bucket = ct->purge_bucket,
             offset = ct->purge_offset;
    long long now = time_msec();
    struct hmap_node *node;
    struct conn *conn;
    unsigned i;

    ovs_mutex_lock(&ct->mutex[bucket_ext]);

    for (i = 0; i < CONNTRACK_PURGE_NUM; i++) {

        node = hmap_at_position(&ct->connections[bucket_ext],
                                &bucket, &offset);
        if (!node) {
            ovs_mutex_unlock(&ct->mutex[bucket_ext]);
            bucket_ext++;
            if (bucket_ext < CONNTRACK_BUCKETS) {
                ovs_mutex_lock(&ct->mutex[bucket_ext]);
                continue;
            } else {
                break;
        ...
        if (conn_expired(conn, now)) {
            hmap_remove(&ct->connections[bucket_ext], &conn->node);
            delete_conn(conn);
        } else {
            ct->purge_bucket = bucket;
            ct->purge_offset = offset;
        }
    }

    /* XXX UGLY */
    if (i == CONNTRACK_PURGE_NUM) {
        ovs_mutex_unlock(&ct->mutex[bucket_ext]);
    }

    ct->purge_bucket_ext = bucket_ext;
    ct->purge_bucket = bucket;
    ct->purge_offset = offset;
}
                   ...
                   uint32_t val, uint32_t mask)
{
    ...
    for (i = 0; i < cnt; i++) {
        struct conn *conn = pkts[i]->ct_conn;

        if (!(pkts[i]->md.ct_state & OVS_CS_F_TRACKED) || !conn) {
            continue;
        }

        pkts[i]->md.ct_mark = val | (pkts[i]->md.ct_mark & ~(mask));
        /* XXX fill hash */
        ovs_mutex_lock(&ct->mutex[conn->hash % CONNTRACK_BUCKETS]);
        conn->mark = pkts[i]->md.ct_mark;
        ovs_mutex_unlock(&ct->mutex[conn->hash % CONNTRACK_BUCKETS]);
    }
}
                 ...
                 uint16_t zone)
{
    ...
    key->hash = conn_key_hash(&key->key);
}

/* Symmetric */
static uint32_t
