...
/* Extracts the numa node and core info from the 'dummy_config'.  This is
 * useful for testing purposes.  The function must be called only once,
 * before ovs_numa_init().  Every subsequent call to ovs_numa_init() will
 * have no effect.
 *
 * The format of 'dummy_config' is explained in the comment above
 * discover_numa_and_core_dummy().*/
void
ovs_numa_init_dummy(const char *dummy_config)
{
    dummy_numa = true;
    ovs_assert(dummy_config);
    ovs_assert(ovs_numa_init__(dummy_config));
}

/* Extracts the numa node and core info from sysfs. */
void
ovs_numa_init(void)
{
